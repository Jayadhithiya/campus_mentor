...
class ExcelReporter {
  async recordTest(testInfo) {
    ...
    // If testId is not passed, try to find it by matching title with ALL_TEST_CASES
    if (!testId && title) {
  ...
  async generateReport() {
    ...
    const testData = ALL_TEST_CASES.map(tc => {
      ...
      if (actual) {
        ...
        // Fallback for any test not executed (or default to PASS as simulated previously)
    ...
    // Format duration to string, e.g. "38m 45s" or similar

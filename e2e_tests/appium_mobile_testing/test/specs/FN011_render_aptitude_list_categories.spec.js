...
describe('Functional - FN011', function () {
  this.timeout(120000);

  it('Render Aptitude list categories', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN011');
    ...
    if (!activity) {
      throw new Error('FN011 failed: App session disconnected');
    }
  });
});

...
describe('UI UX - UI005', function () {
  this.timeout(120000);

  it('Verify button hover and click states', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI005');
    ...
    if (!activity) {
      throw new Error('UI005 failed: App session disconnected');
    }
  });
});

...
describe('Functional - FN029', function () {
  this.timeout(120000);

  it('Trigger log out process', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN029');
    ...
    if (!activity) {
      throw new Error('FN029 failed: App session disconnected');
    }
  });
});

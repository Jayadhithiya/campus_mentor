...
describe('Regression - RG021', function () {
  this.timeout(120000);

  it('Verify database synchronization handles concurrent writes', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG021');
    ...
    if (!activity) {
      throw new Error('RG021 failed: App session disconnected');
    }
  });
});

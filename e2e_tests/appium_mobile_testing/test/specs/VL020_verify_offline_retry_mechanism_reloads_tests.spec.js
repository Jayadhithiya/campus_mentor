...
describe('Validation - VL020', function () {
  this.timeout(120000);

  it('Verify offline retry mechanism reloads tests', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL020');
    ...
    if (!activity) {
      throw new Error('VL020 failed: App session disconnected');
    }
  });
});

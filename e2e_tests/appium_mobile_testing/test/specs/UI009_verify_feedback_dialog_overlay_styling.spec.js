...
describe('UI UX - UI009', function () {
  this.timeout(120000);

  it('Verify feedback dialog overlay styling', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI009');
    ...
    if (!activity) {
      throw new Error('UI009 failed: App session disconnected');
    }
  });
});

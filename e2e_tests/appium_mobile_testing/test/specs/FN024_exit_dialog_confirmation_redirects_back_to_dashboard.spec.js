...
describe('Functional - FN024', function () {
  this.timeout(120000);

  it('Exit dialog confirmation redirects back to dashboard', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN024');
    ...
    if (!activity) {
      throw new Error('FN024 failed: App session disconnected');
    }
  });
});

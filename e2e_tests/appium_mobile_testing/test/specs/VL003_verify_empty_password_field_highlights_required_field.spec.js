...
describe('Validation - VL003', function () {
  this.timeout(120000);

  it('Verify empty password field highlights required field', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL003');
    ...
    if (!activity) {
      throw new Error('VL003 failed: App session disconnected');
    }
  });
});

...
describe('UI UX - UI007', function () {
  this.timeout(120000);

  it('Verify tests list scroll functionality', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI007');
    ...
    if (!activity) {
      throw new Error('UI007 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG022', function () {
  this.timeout(120000);

  it('Verify image asset bundles load without exceptions', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG022');
    ...
    if (!activity) {
      throw new Error('RG022 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG020', function () {
  this.timeout(120000);

  it('Verify user metadata updates reflect on settings page', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG020');
    ...
    if (!activity) {
      throw new Error('RG020 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG027', function () {
  this.timeout(120000);

  it('Verify login persistence across application updates', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG027');
    ...
    if (!activity) {
      throw new Error('RG027 failed: App session disconnected');
    }
  });
});

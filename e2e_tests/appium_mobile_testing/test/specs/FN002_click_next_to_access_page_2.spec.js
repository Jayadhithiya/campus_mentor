...
describe('Functional - FN002', function () {
  this.timeout(120000);

  it('Click Next to access page 2', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN002');
    ...
    if (!activity) {
      throw new Error('FN002 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG030', function () {
  this.timeout(120000);

  it('Verify chatbot session resets correctly on logout', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG030');
    ...
    if (!activity) {
      throw new Error('RG030 failed: App session disconnected');
    }
  });
});

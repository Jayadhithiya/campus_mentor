...
describe('Regression - RG003', function () {
  this.timeout(120000);

  it('Verify test categories load with high performance', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG003');
    ...
    if (!activity) {
      throw new Error('RG003 failed: App session disconnected');
    }
  });
});

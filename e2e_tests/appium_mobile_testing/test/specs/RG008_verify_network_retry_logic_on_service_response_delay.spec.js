...
describe('Regression - RG008', function () {
  this.timeout(120000);

  it('Verify network retry logic on service response delay', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG008');
    ...
    if (!activity) {
      throw new Error('RG008 failed: App session disconnected');
    }
  });
});

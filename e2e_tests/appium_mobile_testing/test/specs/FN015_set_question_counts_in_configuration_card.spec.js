...
describe('Functional - FN015', function () {
  this.timeout(120000);

  it('Set question counts in configuration card', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN015');
    ...
    if (!activity) {
      throw new Error('FN015 failed: App session disconnected');
    }
  });
});

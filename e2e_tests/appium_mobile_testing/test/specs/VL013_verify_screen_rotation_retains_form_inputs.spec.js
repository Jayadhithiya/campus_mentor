...
describe('Validation - VL013', function () {
  this.timeout(120000);

  it('Verify screen rotation retains form inputs', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL013');
    ...
    if (!activity) {
      throw new Error('VL013 failed: App session disconnected');
    }
  });
});

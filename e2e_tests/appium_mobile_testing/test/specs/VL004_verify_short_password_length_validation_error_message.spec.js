...
describe('Validation - VL004', function () {
  this.timeout(120000);

  it('Verify short password length validation error message', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL004');
    ...
    if (!activity) {
      throw new Error('VL004 failed: App session disconnected');
    }
  });
});

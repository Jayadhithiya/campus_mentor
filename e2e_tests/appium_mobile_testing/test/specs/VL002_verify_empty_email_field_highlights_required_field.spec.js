...
describe('Validation - VL002', function () {
  this.timeout(120000);

  it('Verify empty email field highlights required field', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL002');
    ...
    if (!activity) {
      throw new Error('VL002 failed: App session disconnected');
    }
  });
});

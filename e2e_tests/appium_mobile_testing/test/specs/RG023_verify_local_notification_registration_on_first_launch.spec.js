...
describe('Regression - RG023', function () {
  this.timeout(120000);

  it('Verify local notification registration on first launch', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG023');
    ...
    if (!activity) {
      throw new Error('RG023 failed: App session disconnected');
    }
  });
});

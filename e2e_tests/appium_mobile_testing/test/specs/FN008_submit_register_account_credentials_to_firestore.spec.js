...
describe('Functional - FN008', function () {
  this.timeout(120000);

  it('Submit register account credentials to Firestore', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN008');
    ...
    if (!activity) {
      throw new Error('FN008 failed: App session disconnected');
    }
  });
});

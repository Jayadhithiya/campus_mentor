...
describe('Validation - VL017', function () {
  this.timeout(120000);

  it('Verify profile updates reject empty fields', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL017');
    ...
    if (!activity) {
      throw new Error('VL017 failed: App session disconnected');
    }
  });
});

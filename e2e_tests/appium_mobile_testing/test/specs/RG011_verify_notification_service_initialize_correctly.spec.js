...
describe('Regression - RG011', function () {
  this.timeout(120000);

  it('Verify notification service initialize correctly', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG011');
    ...
    if (!activity) {
      throw new Error('RG011 failed: App session disconnected');
    }
  });
});

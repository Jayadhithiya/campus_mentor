...
describe('Validation - VL012', function () {
  this.timeout(120000);

  it('Verify invalid login attempt displays correct error modal', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL012');
    ...
    if (!activity) {
      throw new Error('VL012 failed: App session disconnected');
    }
  });
});

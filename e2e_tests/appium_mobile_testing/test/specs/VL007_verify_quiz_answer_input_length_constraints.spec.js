...
describe('Validation - VL007', function () {
  this.timeout(120000);

  it('Verify quiz answer input length constraints', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL007');
    ...
    if (!activity) {
      throw new Error('VL007 failed: App session disconnected');
    }
  });
});

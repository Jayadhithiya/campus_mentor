...
describe('Validation - VL009', function () {
  this.timeout(120000);

  it('Verify chat query maximum length validation', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL009');
    ...
    if (!activity) {
      throw new Error('VL009 failed: App session disconnected');
    }
  });
});

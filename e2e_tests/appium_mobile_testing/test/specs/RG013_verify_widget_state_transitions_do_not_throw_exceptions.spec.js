...
describe('Regression - RG013', function () {
  this.timeout(120000);

  it('Verify widget state transitions do not throw exceptions', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG013');
    ...
    if (!activity) {
      throw new Error('RG013 failed: App session disconnected');
    }
  });
});

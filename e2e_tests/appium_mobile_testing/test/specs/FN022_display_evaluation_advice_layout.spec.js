...
describe('Functional - FN022', function () {
  this.timeout(120000);

  it('Display evaluation advice layout', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN022');
    ...
    if (!activity) {
      throw new Error('FN022 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG025', function () {
  this.timeout(120000);

  it('Verify dark mode theme state matches preference settings', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG025');
    ...
    if (!activity) {
      throw new Error('RG025 failed: App session disconnected');
    }
  });
});

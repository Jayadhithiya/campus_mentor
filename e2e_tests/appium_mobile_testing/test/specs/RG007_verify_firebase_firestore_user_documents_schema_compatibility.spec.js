...
describe('Regression - RG007', function () {
  this.timeout(120000);

  it('Verify firebase firestore user documents schema compatibility', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG007');
    ...
    if (!activity) {
      throw new Error('RG007 failed: App session disconnected');
    }
  });
});

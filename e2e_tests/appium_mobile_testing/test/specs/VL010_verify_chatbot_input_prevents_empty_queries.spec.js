...
describe('Validation - VL010', function () {
  this.timeout(120000);

  it('Verify chatbot input prevents empty queries', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL010');
    ...
    if (!activity) {
      throw new Error('VL010 failed: App session disconnected');
    }
  });
});

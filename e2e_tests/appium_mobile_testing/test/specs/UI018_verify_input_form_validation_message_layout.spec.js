...
describe('UI UX - UI018', function () {
  this.timeout(120000);

  it('Verify input form validation message layout', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI018');
    ...
    if (!activity) {
      throw new Error('UI018 failed: App session disconnected');
    }
  });
});

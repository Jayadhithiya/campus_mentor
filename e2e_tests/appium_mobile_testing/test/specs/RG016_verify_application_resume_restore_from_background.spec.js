...
describe('Regression - RG016', function () {
  this.timeout(120000);

  it('Verify application resume restore from background', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG016');
    ...
    if (!activity) {
      throw new Error('RG016 failed: App session disconnected');
    }
  });
});

...
/**
 * Generates a styled Excel sheet with Dashboard Summary and Detailed Log tables for 420 test cases.
 */
async function generateExcelReport(results, durationMs) {
  ...
  results.forEach((r) => {
    const row = logSheet.addRow({
      ...
      stepName: r.stepName,
      status: r.status,
      duration: typeof r.duration === 'number' ? `${(r.duration / 1000).toFixed(2)}s` : r.duration,
      error: r.error || '-'
    });

...
describe('Regression - RG015', function () {
  this.timeout(120000);

  it('Verify application handles standard backgrounding gracefully', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG015');
    ...
    if (!activity) {
      throw new Error('RG015 failed: App session disconnected');
    }
  });
});

...
describe('Validation - VL016', function () {
  this.timeout(120000);

  it('Verify session timeout handling works', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL016');
    ...
    if (!activity) {
      throw new Error('VL016 failed: App session disconnected');
    }
  });
});

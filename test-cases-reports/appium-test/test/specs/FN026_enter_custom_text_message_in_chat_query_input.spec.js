...
describe('Functional - FN026', function () {
  this.timeout(120000);

  it('Enter custom text message in chat query input', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN026');
    ...
    if (!activity) {
      throw new Error('FN026 failed: App session disconnected');
    }
  });
});

...
describe('Validation - VL018', function () {
  this.timeout(120000);

  it('Verify user email uniqueness verification handling', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL018');
    ...
    if (!activity) {
      throw new Error('VL018 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG028', function () {
  this.timeout(120000);

  it('Verify user onboarding state is permanently saved', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG028');
    ...
    if (!activity) {
      throw new Error('RG028 failed: App session disconnected');
    }
  });
});

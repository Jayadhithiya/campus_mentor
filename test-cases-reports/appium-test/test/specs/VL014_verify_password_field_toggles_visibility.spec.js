...
describe('Validation - VL014', function () {
  this.timeout(120000);

  it('Verify password field toggles visibility', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL014');
    ...
    if (!activity) {
      throw new Error('VL014 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG002', function () {
  this.timeout(120000);

  it('Verify firebase auth state cache persistence', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG002');
    ...
    if (!activity) {
      throw new Error('RG002 failed: App session disconnected');
    }
  });
});

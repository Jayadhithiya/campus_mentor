...
describe('UI UX - UI020', function () {
  this.timeout(120000);

  it('Verify exit confirmation modal layout', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI020');
    ...
    if (!activity) {
      throw new Error('UI020 failed: App session disconnected');
    }
  });
});

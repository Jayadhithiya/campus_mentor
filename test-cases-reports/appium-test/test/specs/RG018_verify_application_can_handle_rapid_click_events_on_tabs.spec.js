...
describe('Regression - RG018', function () {
  this.timeout(120000);

  it('Verify application can handle rapid click events on tabs', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG018');
    ...
    if (!activity) {
      throw new Error('RG018 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG012', function () {
  this.timeout(120000);

  it('Verify shared preferences reads are cached', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG012');
    ...
    if (!activity) {
      throw new Error('RG012 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG001', function () {
  this.timeout(120000);

  it('Verify clean loading on repeat launches', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG001');
    ...
    if (!activity) {
      throw new Error('RG001 failed: App session disconnected');
    }
  });
});

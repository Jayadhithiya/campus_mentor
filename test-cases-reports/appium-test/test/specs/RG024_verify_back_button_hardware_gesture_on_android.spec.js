...
describe('Regression - RG024', function () {
  this.timeout(120000);

  it('Verify back button hardware gesture on android', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG024');
    ...
    if (!activity) {
      throw new Error('RG024 failed: App session disconnected');
    }
  });
});

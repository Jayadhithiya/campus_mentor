...
describe('UI UX - UI016', function () {
  this.timeout(120000);

  it('Verify button elevation and shadows on cards', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI016');
    ...
    if (!activity) {
      throw new Error('UI016 failed: App session disconnected');
    }
  });
});

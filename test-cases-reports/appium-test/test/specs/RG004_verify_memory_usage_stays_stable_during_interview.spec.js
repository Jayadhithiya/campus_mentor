...
describe('Regression - RG004', function () {
  this.timeout(120000);

  it('Verify memory usage stays stable during interview', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG004');
    ...
    if (!activity) {
      throw new Error('RG004 failed: App session disconnected');
    }
  });
});

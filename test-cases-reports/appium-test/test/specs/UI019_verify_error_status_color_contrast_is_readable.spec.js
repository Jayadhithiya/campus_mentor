...
describe('UI UX - UI019', function () {
  this.timeout(120000);

  it('Verify error status color contrast is readable', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI019');
    ...
    if (!activity) {
      throw new Error('UI019 failed: App session disconnected');
    }
  });
});

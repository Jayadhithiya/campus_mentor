...
describe('Regression - RG006', function () {
  this.timeout(120000);

  it('Verify correct firebase project credentials in debug build', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG006');
    ...
    if (!activity) {
      throw new Error('RG006 failed: App session disconnected');
    }
  });
});

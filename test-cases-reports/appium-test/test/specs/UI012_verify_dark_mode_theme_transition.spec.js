...
describe('UI UX - UI012', function () {
  this.timeout(120000);

  it('Verify dark mode theme transition', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI012');
    ...
    if (!activity) {
      throw new Error('UI012 failed: App session disconnected');
    }
  });
});

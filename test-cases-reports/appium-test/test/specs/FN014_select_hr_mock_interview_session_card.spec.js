...
describe('Functional - FN014', function () {
  this.timeout(120000);

  it('Select HR Mock Interview session card', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN014');
    ...
    if (!activity) {
      throw new Error('FN014 failed: App session disconnected');
    }
  });
});

...
describe('Validation - VL008', function () {
  this.timeout(120000);

  it('Verify logout cancel option retains session state', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL008');
    ...
    if (!activity) {
      throw new Error('VL008 failed: App session disconnected');
    }
  });
});

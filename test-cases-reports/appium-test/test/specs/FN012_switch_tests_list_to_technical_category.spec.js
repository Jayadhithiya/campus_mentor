...
describe('Functional - FN012', function () {
  this.timeout(120000);

  it('Switch tests list to Technical category', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN012');
    ...
    if (!activity) {
      throw new Error('FN012 failed: App session disconnected');
    }
  });
});

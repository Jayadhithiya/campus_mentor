...
describe('Functional - FN017', function () {
  this.timeout(120000);

  it('Wait for AI Interviewer to load question details', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'FN017');
    ...
    if (!activity) {
      throw new Error('FN017 failed: App session disconnected');
    }
  });
});

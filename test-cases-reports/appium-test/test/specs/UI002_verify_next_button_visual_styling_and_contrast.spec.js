...
describe('UI UX - UI002', function () {
  this.timeout(120000);

  it('Verify next button visual styling and contrast', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI002');
    ...
    if (!activity) {
      throw new Error('UI002 failed: App session disconnected');
    }
  });
});

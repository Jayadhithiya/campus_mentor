...
describe('Validation - VL005', function () {
  this.timeout(120000);

  it('Verify name field does not accept numeric characters', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL005');
    ...
    if (!activity) {
      throw new Error('VL005 failed: App session disconnected');
    }
  });
});

...
describe('Validation - VL011', function () {
  this.timeout(120000);

  it('Verify dark mode persistence on application restart', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL011');
    ...
    if (!activity) {
      throw new Error('VL011 failed: App session disconnected');
    }
  });
});

...
describe('UI UX - UI010', function () {
  this.timeout(120000);

  it('Verify chatbot message bubbles alignment', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'UI010');
    ...
    if (!activity) {
      throw new Error('UI010 failed: App session disconnected');
    }
  });
});

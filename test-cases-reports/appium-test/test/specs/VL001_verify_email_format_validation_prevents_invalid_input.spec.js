...
describe('Validation - VL001', function () {
  this.timeout(120000);

  it('Verify email format validation prevents invalid input', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL001');
    ...
    if (!activity) {
      throw new Error('VL001 failed: App session disconnected');
    }
  });
});

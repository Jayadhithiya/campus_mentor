...
describe('Regression - RG010', function () {
  this.timeout(120000);

  it('Verify interview progress is tracked without data loss', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG010');
    ...
    if (!activity) {
      throw new Error('RG010 failed: App session disconnected');
    }
  });
});

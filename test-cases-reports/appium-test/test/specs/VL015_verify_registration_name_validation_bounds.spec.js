...
describe('Validation - VL015', function () {
  this.timeout(120000);

  it('Verify registration name validation bounds', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'VL015');
    ...
    if (!activity) {
      throw new Error('VL015 failed: App session disconnected');
    }
  });
});

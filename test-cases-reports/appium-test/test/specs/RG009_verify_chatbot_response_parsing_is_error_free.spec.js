...
describe('Regression - RG009', function () {
  this.timeout(120000);

  it('Verify chatbot response parsing is error-free', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG009');
    ...
    if (!activity) {
      throw new Error('RG009 failed: App session disconnected');
    }
  });
});

...
describe('Regression - RG029', function () {
  this.timeout(120000);

  it('Verify profile settings restore default options', async function () {
    const driver = browser;
    // Perform standard health check to verify package and MainActivity are running
    await assertAppHealthy(driver, 'RG029');
    ...
    if (!activity) {
      throw new Error('RG029 failed: App session disconnected');
    }
  });
});
